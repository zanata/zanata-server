...
import { Icons, Icon, Input, Loader, Select } from 'zanata-ui'
...
var SystemGlossary = React.createClass({
  ...
  render: function () {
    ...
    let selectedTransLocale = this.state.selectedTransLocale
      ? this.state.selectedTransLocale.value : ''
    let uploadSection
    let newEntrySection
    let messageModal
    ...
    let optionOutput = (op) => {
      ...
      <span className='D(f) Ai(c) Jc(sb)'>
        <span className='fxauto LineClamp(1)' title={op.label}>
          {op.label}
        </span>
        <span className='fxnone Pstart(re) Ta(end) Maw(r4) LineClamp(1)'>
          {op.value}
        </span>
        <span className='fxnone C(muted) Pstart(re) Ta(end) LineClamp(1) W(r2)'>
          {count}
        </span>
      </span>
      )
    ...
    var allowNewEntry = this.state.canAddNewEntry &&
      !_.isUndefined(srcLocale) && !_.isNull(srcLocale)

    if (allowNewEntry) {
      uploadSection = (
        <ImportModal srcLocale={this.state.srcLocale}
          transLocales={this.state.locales} />
      )
      newEntrySection = (
        <NewEntryModal className='ml1/2' srcLocale={this.state.srcLocale} />
      )
    }
    var loader
