...
public class EditorOptionsPresenter extends
        WidgetPresenter<EditorOptionsDisplay> implements
    ...
    public void onEnterSaveOptionChanged(Boolean enterSaveApproved) {
        if (userOptionsService.getConfigHolder().getState()
                .isEnterSavesApproved() != enterSaveApproved) {
            userOptionsService.getConfigHolder().setEnterSavesApproved(
                    enterSaveApproved);
            eventBus.fireEvent(UserConfigChangeEvent.EDITOR_CONFIG_CHANGE_EVENT);
        }
    }
    ...
    public void onEditorButtonsOptionChanged(Boolean editorButtons) {
        if (userOptionsService.getConfigHolder().getState().isDisplayButtons() != editorButtons) {
            userOptionsService.getConfigHolder().setDisplayButtons(
                    editorButtons);
            eventBus.fireEvent(UserConfigChangeEvent.EDITOR_CONFIG_CHANGE_EVENT);
        }
    }
    ...
    @Override
    public void
            onShowSaveApprovedWarningChanged(Boolean showSaveApprovedWarning) {
        if (userOptionsService.getConfigHolder().getState()
                .isShowSaveApprovedWarning() != showSaveApprovedWarning) {
            userOptionsService.getConfigHolder().setShowSaveApprovedWarning(
                    showSaveApprovedWarning);
            eventBus.fireEvent(UserConfigChangeEvent.EDITOR_CONFIG_CHANGE_EVENT);
        }
    }
    ...
    @Override
    public void onTMOrGlossaryDisplayOptionsChanged(Boolean showTMChkValue,
            Boolean showGlossaryChkValue) {
        UserConfigHolder.ConfigurationState state =
                userOptionsService.getConfigHolder().getState();
    ...
    public void onDisplayTransUnitDetailsOptionChanged(
        ...
        if (userOptionsService.getConfigHolder().getState()
                .isShowOptionalTransUnitDetails() != showTransUnitDetailsChkValue) {
            userOptionsService.getConfigHolder()
                    .setShowOptionalTransUnitDetails(
    ...
    public void onReloadUserConfigUI(ReloadUserConfigUIEvent event) {
        if (event.getView() == MainView.Editor) {
            display.setOptionsState(userOptionsService.getConfigHolder()
                    .getState());
        }
    }
