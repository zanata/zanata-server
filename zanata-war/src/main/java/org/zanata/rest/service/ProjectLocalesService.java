...
public class ProjectLocalesService implements ProjectLocalesResource {
    ...
    public Response get() {
        ...
        List<LocaleDetails> locales =
                Lists.newArrayListWithExpectedSize(supportedLocales.size());
        ...
        for (HLocale hLocale : supportedLocales) {
            ...
            locales.add(new LocaleDetails(id, name, alias));
        }

        Type genericType = new GenericType<List<LocaleDetails>>() {
        }.getGenericType();
        Object entity =
                new GenericEntity<List<LocaleDetails>>(locales, genericType);
