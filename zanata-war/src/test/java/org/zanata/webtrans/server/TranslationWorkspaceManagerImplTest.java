...
public class TranslationWorkspaceManagerImplTest
{
   ...
   public void testRegisterInvalidWorkspace() throws Exception
   {
      ...
      when(projectIterationDAO.getBySlug(workspaceId.getProjectIterationId().getProjectSlug(), workspaceId.getProjectIterationId().getIterationSlug())).thenReturn(null);

      manager.getOrRegisterWorkspace(workspaceId);
   }

   @Test(expectedExceptions = NoSuchWorkspaceException.class)
   public void testRegisterWorkspaceForObsoleteProjectIteration() throws Exception
   {
      WorkspaceId workspaceId = TestFixture.workspaceId();
      HProjectIteration projectIteration = makeHProjectIteration("a", "1");
      projectIteration.getProject().setStatus(EntityStatus.OBSOLETE);
      when(projectIterationDAO.getBySlug(workspaceId.getProjectIterationId().getProjectSlug(), workspaceId.getProjectIterationId().getIterationSlug())).thenReturn(projectIteration);
   ...
   {
      WorkspaceId workspaceId = TestFixture.workspaceId();
      when(projectIterationDAO.getBySlug(workspaceId.getProjectIterationId().getProjectSlug(), workspaceId.getProjectIterationId().getIterationSlug())).thenReturn(makeHProjectIteration("a", "1"));
      when(localeServiceImpl.getByLocaleId(workspaceId.getLocaleId())).thenReturn(null);
   ...
   {
      WorkspaceId workspaceId = TestFixture.workspaceId();
      when(projectIterationDAO.getBySlug(workspaceId.getProjectIterationId().getProjectSlug(), workspaceId.getProjectIterationId().getIterationSlug())).thenReturn(makeHProjectIteration("project", "1"));
   ...
   public void testRegisterNewWorkspace() throws Exception
   {
      ...
      when(projectIterationDAO.getBySlug(workspaceId.getProjectIterationId().getProjectSlug(), workspaceId.getProjectIterationId().getIterationSlug())).thenReturn(makeHProjectIteration("project", "master"));
      HLocale hLocale = new HLocale(LocaleId.DE);
      hLocale.setActive(true);
      when(localeServiceImpl.getByLocaleId(workspaceId.getLocaleId())).thenReturn(hLocale);

      TranslationWorkspace workspace = manager.getOrRegisterWorkspace(workspaceId);
      ...
      assertThat(context.getWorkspaceName(), Matchers.equalTo("project (master)"));
   }
   ...
   public void testGetRegisteredNewWorkspace() throws Exception
   {
      ...
      TranslationWorkspace anotherWorkspace = manager.getOrRegisterWorkspace(workspaceId);
   ...
   {
      HProjectIteration projectIteration = makeHProjectIteration("project", "master");
      WorkspaceId workspaceId = TestFixture.workspaceId(LocaleId.DE);
      when(projectIterationDAO.getBySlug(workspaceId.getProjectIterationId().getProjectSlug(), workspaceId.getProjectIterationId().getIterationSlug())).thenReturn(projectIteration);
      HLocale hLocale = new HLocale(LocaleId.DE);
      hLocale.setActive(true);
      ...
      when(validationServiceImpl.getValidationAction(projectIteration.getProject().getSlug(), projectIteration.getSlug())).thenReturn(new ArrayList<ValidationAction>());
   ...
   public void testProjectUpdate() throws Exception
   {
      ...
      TranslationWorkspaceManagerImpl spy = spy(manager);
      doNothing().when(spy).projectIterationUpdate(Mockito.any(HProjectIteration.class));
   ...
   public void testExitWorkspaceWithAccount() throws Exception
   {
      ...
      ArrayList<EditorClientId> editorClientIds = Lists.newArrayList(new EditorClientId("sessionId", 1L), new EditorClientId("sessionId", 2L));
      when(mockWorkspace.removeEditorClients("sessionId")).thenReturn(editorClientIds);
      spy.getOrRegisterWorkspace(workspaceId);
      ...
      assertThat(exitWorkspace1.getEditorClientId(), Matchers.equalTo(editorClientIds.get(0)));
      assertThat(exitWorkspace1.getPerson().getName(), Matchers.equalTo("patrick"));
      verify(mockWorkspace).publish(exitWorkspace1);

      ExitWorkspace exitWorkspace2 = eventCaptor.getAllValues().get(1);
      assertThat(exitWorkspace2.getEditorClientId(), Matchers.equalTo(editorClientIds.get(1)));
      verify(mockWorkspace).publish(exitWorkspace2);
      verify(gravatarServiceImpl, times(2)).getUserImageUrl(16, "pahuang@redhat.com");
   }
   ...
   public void testExitWorkspaceWithNullAccount() throws Exception
   {
      ...
      ExitWorkspace exitWorkspace1 = eventCaptor.getAllValues().get(0);
      assertThat(exitWorkspace1.getEditorClientId(), Matchers.equalTo(editorClientIds.get(0)));
      assertThat(exitWorkspace1.getPerson().getName(), Matchers.equalTo("<unknown>"));
      verify(mockWorkspace).publish(exitWorkspace1);
